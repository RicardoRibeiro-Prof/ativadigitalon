(function () {
  ...
  async function searchProspects(event) {
    ...
    if (!segment || !/^.{2,},\s*[A-Za-z]{2}$/.test(city)) {
      showToast("Informe a cidade completa e a sigla do estado. Ex.: São Raimundo Nonato, PI.", "error");
      return;
    }
    ...
    try {
      ...
      params.set("versao", APP_VERSION);
      const response = await fetch(`/api/prospectar?${params}`, { cache: "no-store" });
      ...
      byId("prospectStatus").innerHTML = data.warning
        ? `<div class="search-notice"><span aria-hidden="true">ⓘ</span><p>${escapeHTML(data.warning)}</p></div>`
        : "";
  ...
  function renderProspectResults(data) {
    ...
    byId("prospectAttribution").innerHTML = data.source === "Receita Federal"
      ? 'Dados públicos do CNPJ — <a href="https://www.gov.br/receitafederal/pt-br/assuntos/cadastros/cnpj" target="_blank" rel="noopener noreferrer">Receita Federal</a>, tratados pela <a href="https://basedosdados.org/" target="_blank" rel="noopener noreferrer">Base dos Dados</a>.'
      : 'Dados © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">colaboradores do OpenStreetMap</a>.';
  ...
  function prospectCard(prospect, index) {
    ...
    const mapUrl = Number.isFinite(prospect.latitude) && Number.isFinite(prospect.longitude)
      ...
      : "";
    const extraInfo = [prospect.cnpj ? `CNPJ ${prospect.cnpj}` : "", prospect.companySize || ""].filter(Boolean).join(" · ");
    const digitalContact = prospect.email || prospect.instagram || "";
    const digitalLabel = prospect.email ? "E-mail" : "Instagram";
    return `<article class="prospect-card${duplicate ? " duplicate" : ""}">
      ...
      <div class="prospect-main">
        ...
        <p>${escapeHTML(prospect.address || "Endereço não informado na fonte")}${extraInfo ? `<br>${escapeHTML(extraInfo)}` : ""}</p>
        <div class="prospect-contact-grid">
          ...
          <span class="contact-chip ${digitalContact ? "has-data" : "missing"}"><b>${digitalLabel}</b>${escapeHTML(digitalContact || "Não disponível")}</span>
  ...
  function addSelectedProspects() {
    ...
    const imported = selected.map((prospect, offset) => ({
      ...
      notes: [`Importado automaticamente de ${prospect.source || "fonte pública"}.`, prospect.cnpj ? `CNPJ: ${prospect.cnpj}` : "", prospect.address ? `Endereço: ${prospect.address}` : "", prospect.email ? `E-mail: ${prospect.email}` : "", prospect.website ? `Site: ${prospect.website}` : "Sem site cadastrado — oportunidade para oferta.", prospect.instagram ? `Instagram: ${prospect.instagram}` : ""].filter(Boolean).join("\n"),
      createdAt: now,
      updatedAt: now
    }));
